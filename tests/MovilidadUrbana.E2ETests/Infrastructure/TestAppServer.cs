...
public class TestAppServer
{
    ...
    private static async Task PublicarLaAplicacionAsync(string navTarget)
    {
        ...
        if (!File.Exists(proyecto))
        {
            TestContext.Progress.WriteLine(
                $"No se encontró {proyecto}: se probará con lo que ya haya en «{navTarget}».");
            return;
        }

        TestContext.Progress.WriteLine($"Publicando la aplicación bajo prueba en «{navTarget}»…");
        ...
        foreach (var argumento in new[] { "publish", proyecto, "--configuration", "Release", "--output", navTarget })
        {
    ...
    private static async Task EsperarAQueEscucheAsync(int puerto)
    {
        ...
        while (DateTime.UtcNow < limite)
        {
            ...
            try
            {
                ...
                await client.ConnectAsync("127.0.0.1", puerto);
                return;
            }
