...
/// </summary>
[TestFixture]
public class LocalidadRulesTests
{
    ...
    [TestCase("", ExpectedResult = false)]
    [TestCase(null, ExpectedResult = false)]
    public bool ProvinciaValida(string? provincia) => LocalidadRules.ProvinciaValida(provincia);
    ...
    [TestCase("", ExpectedResult = false)]
    [TestCase(null, ExpectedResult = false)]
    public bool CodigoPostalValido(string? codigoPostal) => LocalidadRules.CodigoPostalValido(codigoPostal);
    ...
    [TestCase(null, ExpectedResult = false)]
    public bool HabitantesValidos(int? habitantes) => LocalidadRules.HabitantesValidos(habitantes);
    ...
    public void MismaLocalidadCompara()
    {
        Assert.Multiple(() =>
        {
            Assert.That(LocalidadRules.MismaLocalidad("corrientes", "Corrientes", "Corrientes", "Corrientes"), Is.True);
            Assert.That(LocalidadRules.MismaLocalidad(" Corrientes ", "Corrientes", "Corrientes", "Corrientes"), Is.True);
            Assert.That(LocalidadRules.MismaLocalidad("Corrientes", "Corrientes", "Corrientes", "Chaco"), Is.False);
            Assert.That(LocalidadRules.MismaLocalidad("Goya", "Corrientes", "Corrientes", "Corrientes"), Is.False);
        });
    }
    ...
    public void LaProvinciaSeComparaDeFormaOrdinal() =>
        Assert.That(LocalidadRules.MismaLocalidad("Goya", "corrientes", "Goya", "Corrientes"), Is.False);
}
