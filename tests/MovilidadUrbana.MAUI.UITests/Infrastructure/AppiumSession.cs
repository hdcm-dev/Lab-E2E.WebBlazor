...
{
    public const string AppPackage = "ar.lab.movilidadurbana";
    ...
    public void Open()
    {
        ...
        opciones.AddAdditionalAppiumOption(AndroidMobileCapabilityType.AppPackage, AppPackage);
        opciones.AddAdditionalAppiumOption(AndroidMobileCapabilityType.AppActivity, $"{AppPackage}.MainActivity");
        ...
        // Recién instalada, la primera apertura puede tener el hilo principal ocupado más de 20 s (JIT en
        // Debug, teléfono lento): mientras tanto UIAutomator2 no consigue el árbol y responde con error.
        // Se reintenta hasta que aparezca la primera pantalla.
        var limite = DateTime.UtcNow + TimeSpan.FromSeconds(120);
        while (true)
        {
            try
            {
                if (_driver.FindElements(OpenQA.Selenium.Appium.MobileBy.Id("lista-localidades")).Count > 0 ||
                    _driver.FindElements(OpenQA.Selenium.Appium.MobileBy.Id("etiqueta-paso")).Count > 0) break;
            }
            catch (OpenQA.Selenium.WebDriverException) { /* árbol todavía no disponible */ }
            if (DateTime.UtcNow > limite) throw new InvalidOperationException("La aplicación no mostró su primera pantalla en 120 s.");
            Thread.Sleep(1000);
        }
    }
