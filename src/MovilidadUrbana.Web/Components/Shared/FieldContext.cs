...
public readonly record struct FieldContext(string Id, string DescribedBy, bool Invalid)
{
    public string InputClass => Invalid ? "mq-input mq-input--invalido" : "mq-input";

    public string SelectClass => Invalid ? "mq-select mq-select--invalido" : "mq-select";
}
